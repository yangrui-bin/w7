...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	public function post() 
	{
		...
		if( $_W["ispost"] ) 
		{
			if( !empty($id) && intval($_GPC["needcount"]) < $item["needcount"] ) 
			{
			...
			if( $item["keyword"] != $data["keyword"] && !empty($keyword) && $keyword["name"] != "ewei_shopv2:task:" . $id ) 
			{
				show_json(0, "关键字已存在!");
			}
			if( !empty($id) ) 
	...
	public function delreward() 
	{
		...
		if( $type == 1 ) 
		{
			...
			if( $datatype == 5 && isset($item["reward_data"]["rec"]["coupon"][$data]) ) 
			{
		...
		if( $type == 2 ) 
		{
			...
			if( $datatype == 5 && isset($item["reward_data"]["sub"]["coupon"][$data]) ) 
			{
	...
	protected function saveCover($keyword, $img, $desc, $title) 
	{
		...
		if( empty($rid) && pdo_insert("rule", array( "uniacid" => $_W["uniacid"], "name" => $entranceName, "module" => "cover", "status" => 1 )) ) 
		{
			$rid = pdo_insertid();
		}
	...
	public function post_new() 
	{
		...
		if( !empty($task["reward"]) ) 
		{
			...
			pdo_update("ewei_shop_task_list", array( "reward" => $task["reward"] ), array( array( "id" => $id, "uniacid" => $uniacid ) ));
			unset($data);
			unset($coupons);
		}
