...
class Index_EweiShopV2Page extends WebPage 
{
	...
	public function searchlist() 
	{
		...
		{
			if( (strexists($item["title"], $keyword) || strexists($item["desc"], $keyword) || strexists($item["keywords"], $keyword) || strexists($item["topsubtitle"], $keyword)) && cv($item["route"]) ) 
			{
				$return_arr[] = $item;
			}
		}
		...
		if( !empty($keyword) ) 
		{
			...
			if( !empty($submenu) ) 
			{
				foreach( $submenu as $index => $submenu_item ) 
				{
					...
					if( (strexists($submenu_item["title"], $keyword) || strexists($submenu_item["desc"], $keyword) || strexists($submenu_item["keywords"], $keyword) || strexists($submenu_item["topsubtitle"], $keyword)) && cv($submenu_item["route"]) ) 
					{
